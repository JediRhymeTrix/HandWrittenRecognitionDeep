...
class DAL:
    ...
    def read_data(self, data_path, label_path):
        ...
        """
        try:
            self.data_set = np.load(data_path)
    ...
    def pre_process_data_set(self, img_dims):
        ...
        """
        train_data = np.array([np.expand_dims(self.resize_padded(np.array(x), img_dims), axis=0)
    ...
    def resize_padded(image, new_shape):
        ...
        delta_width = 150*150 - img.shape[1]
        delta_height = 150*150 - img.shape[0]
